...
class DPAPICMDHelper:
	...
	def add_args(self, parser, live_parser):
		...
		dpapi_credential_group = dpapi_subparsers.add_parser('credential', help='Decrypt credential file with masterkey or key in hex format')
		dpapi_credential_group.add_argument('cred', help='path to credential file')
		dpapi_credential_group.add_argument('--mkf', required=False, help= 'Keyfile generated by the masterkey -o command.')
		dpapi_credential_group.add_argument('--key', required=False, help= 'Key to decrypt in hexadecimal format.')
		...
		# pvk decryption
		dpapi_pvk_group = dpapi_subparsers.add_parser('pvk', help='Decrypt MasterKeyFile with domain backup keys')
		dpapi_pvk_group.add_argument('mkffile', help= 'MasterKeyFile to decrypt')
		dpapi_pvk_group.add_argument('pvkfile', help='PVK file that contains domain backup keys')
	...
	def run(self, args):
		...
		# modified to add "--key" option to use with pvk output key
		elif args.dapi_module == 'credential':
			if args.key is None:
				if args.mkf is None or len(args.mkf) == 0:
					print('MKF or Key must be specified!')
					exit()
				else:
				...
				if args.key is None or len(args.key) == 0:
					...
					cred_blob = dpapi.decrypt_credential_file_with_key(args.cred, args.key)
					print(cred_blob.to_text())
		...
		# pvk decryption
		elif args.dapi_module == 'pvk':
			unprotected_blob = dpapi.decrypt_mkf_with_pvk(args.mkffile, args.pvkfile)
			print('MKF key : %s' % (unprotected_blob['key']))
			#print(unprotected_blob)
