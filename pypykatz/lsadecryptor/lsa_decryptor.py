...
class LsaDecryptor(PackageDecryptor):
	...
	def find_signature(self):
		...
		self.logging.debug('Found candidates on the following positions: %s' % ' '.join(hex(x) for x in fl))
		self.logging.debug('Selecting first one @ 0x%08x' % fl[0])
		return fl[0]

	def get_IV(self, pos):
		self.logging.debug('Reading IV')
	...
	def dump(self):
		self.logging.debug('Recovered LSA encryption keys\n')
		self.logging.debug('IV ({}): {}'.format(len(self.iv), self.iv.hex()))
		self.logging.debug('DES_KEY ({}): {}'.format(len(self.des_key), self.des_key.hex()))
		self.logging.debug('AES_KEY ({}): {}'.format(len(self.aes_key), self.aes_key.hex()))