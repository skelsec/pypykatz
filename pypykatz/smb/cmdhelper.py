...
class SMBCMDHelper:
	...
	def add_args(self, parser, live_parser):
		...
		live_group = live_parser.add_parser('smb', help='SMB (live) client. Use "help" instead of "-h" to get the available subcommands ')
	...
	def run_live(self, args):
		...
		info = get_logon_info()
		la = SMBCMDArgs()
		la.smb_url = 'smb%s+sspi-%s://%s\\%s@%s' % (args.protocol_version, args.authmethod, info['domain'], info['username'], info['logonserver'])
